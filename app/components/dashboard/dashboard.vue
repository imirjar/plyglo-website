<script setup lang="ts">
    ...
    const courses = computed(() => {
      const translatedCourses = tm<Array<{ title: string; description: string; level: string }>>('dashboard.courses')

      return courseMeta.map((course, index) => ({
        ...course,
        ...translatedCourses[index],
      }))
    })

    const buyCourse = (courseId: number) => {
    // Логика покупки курса
    const course = courseMeta.find(c => c.id === courseId)
  ...
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
      <div class="flex items-center space-x-4 mb-6">
        ...
        >
        <div>
          <h1 class="text-2xl font-bold text-gray-800">{{ t('dashboard.userName') }}</h1>
      ...
      <div class="bg-blue-50 rounded-lg p-4">
        <h2 class="text-lg font-semibold text-gray-800 mb-2">{{ t('dashboard.account') }}</h2>
        <p class="text-3xl font-bold text-blue-600">${{ account.balance.toFixed(2) }}</p>
        <p class="text-gray-600 mt-1">{{ t('dashboard.availableBalance') }}</p>
      </div>
    </div>

    <!-- Список курсов -->
    <div class="bg-white rounded-lg shadow-md p-6">
      <h2 class="text-xl font-bold text-gray-800 mb-6">{{ t('dashboard.coursesTitle') }}</h2>
          ...
          <div class="flex justify-between items-start">
            <div>
              ...
              <div class="mt-2 flex items-center">
                ...
                >
                  {{ course.available ? t('dashboard.available') : t('dashboard.unavailable') }}
                </span>
                <span class="ml-2 text-sm font-medium text-gray-700">
                  {{ t(`dashboard.levels.${course.level}`) }}
                </span>
              </div>
            </div>
